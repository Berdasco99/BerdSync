...
if [ -z "$APP_RESOURCES_PATH" ] || [ ! -f "$APP_RESOURCES_PATH/jq" ]; then
  APP_RESOURCES_PATH="$HOME/Library/Application Support/BerdSync"
fi
JQ="$APP_RESOURCES_PATH/jq"
CONFIG_FILE="$HOME/Library/Application Support/BerdSync/config.json"
...
# Only clean up if retention period is greater than zero
